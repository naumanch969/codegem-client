...
export const getGroup = async (req, res, next) => {
    try {
        ...
        const groups = await Group.findById(groupId).populate('admin').populate('codes').populate('sharedCodes.from').populate('sharedCodes.code').exec();
        res.status(200).json(groups)
...
export const createGroups = async (req, res, next) => {
    try {
        ...
        const groups = await Group.create({ ...req.body, admin: req?.user?._id, members: [req?.user?._id] })
        res.status(200).json(groups)
...
export const updateGroups = async (req, res, next) => {
    try {
        const { groupId } = req.params
        const groups = await Group.findByIdAndUpdate(groupId, { $set: { ...req.body } }, { new: true }).populate('admin').exec()
        res.status(200).json(groups)
