...
const Create = () => {
    ...
    return (
        <div className='bg-gray-100 w-full p-8 grid grid-cols-3 gap-8'>
            {
                arr.map((item, index) => (
                    <div onClick={item.onClick} key={index} className={`flex flex-col justify-center items-center p-4 border rounded shadow-xl bg-white group hover:text-copper '
                        } hover:scale-105 transition-all duration-300 border border-gray-800 hover:border-copper cursor-pointer `}>
                        <div className="flex flex-col items-center gap-4 mb-3">
                            <item.icon style={{ fontSize: '5rem' }} />
                            <p className={`text-2xl font-semibold capitalize text-gray-800 group-hover:text-copper `}>
                                {item.name}
                            </p>
                        </div>
                    </div>
