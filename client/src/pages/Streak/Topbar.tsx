...
const Topbar = ({ filters, setFilters }: { filters: any, setFilters: any }) => {
    ...
    return (
        <div className="w-full flex flex-col gap-[1rem] ">
            <CreateStreak open={open} setOpen={setOpen} />

            <div className="flex justify-between items-center " >
                <h1 className="text-[3rem] text-dark-slate-blue font-bold ">Streak</h1>
                ...
                <div className="flex-[7] w-full flex justify-start items-center gap-[1.5rem] border-b-[1px] border-teal-blue  ">
                    {filterButtons.map((item, index) => (
                        <div className=" " key={index}>
                            <button onClick={() => setFilters({ ...filters, streaks: item.toLowerCase() })} className={` ${filters.streaks.toLowerCase() == item.toLowerCase() ? "bg-teal-blue  text-white py-[4px] px-[14px] rounded-t-[4px]  " : "text-dark-slate-blue-lighten "}  `}>
                                {item}
                            </button>
                        </div>
                    ))}
