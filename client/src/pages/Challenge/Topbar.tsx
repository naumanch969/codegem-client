                ...
                <div className="flex-[7] w-full flex justify-start items-center gap-[1.5rem] border-b-[1px] border-teal-blue  ">
                    {filterButtons.map((item, index) => (
                        <div className=" " key={index}>
                            <button onClick={() => setFilters({ ...filters, challenges: item.toLowerCase() })} className={` ${filters.challenges.toLowerCase() == item.toLowerCase() ? "bg-teal-blue  text-white py-[4px] px-[14px] rounded-t-[4px]  " : "text-dark-slate-blue-lighten "}  `}>
                                {item}
                            </button>
                        </div>
                    ))}
