...
const ShareChallenge = ({ open, setOpen, challenge }: { open: boolean, setOpen: any, challenge: Challenge }) => {
    ...
    const { isFetching }: { isFetching: boolean } = useSelector((state: RootState) => state.challenge)
    ...
    const handleShare = () => {
        if (type == 'friend') {
            ...
            dispatch<any>(shareChallenge(challenge, selectedFriends, loggedUser?._id as string, setOpen))
        }
        else {
            if (selectedGroups.length == 0) return alert('Please select someone to share post.')
            dispatch<any>(shareChallengeInGroups(challenge, selectedGroups, loggedUser?._id as string, setOpen))
        }
    }
                ...
                <div className='bg-white md:w-[30rem] sm:w-[60vw] flex flex-col justify-between w-full min-h-[20rem] h-fit max-h-[90vh] overflow-y-scroll rounded-[8px] p-[1rem] ' >
                    ...
                    <div className="flex flex-col">
                        <div className='w-full relative flex justify-between items-center pb-[12px] ' >
                            <h4 className='text-[22px] font-bold text-dark-slate-blue ' >Share Challenge</h4>
