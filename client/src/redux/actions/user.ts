...
export const register = (userData: User, navigate: ReturnType<typeof useNavigate>, setSnackbarText: any): AsyncAction => async (dispatch) => {
    try {
        ...
        console.log('err', err)
        err.response?.data?.message ? dispatch(error(err.response.data.message)) : err.response?.data?.message ? dispatch(error(err.response.data.message)) : dispatch(error(err.message))
    }
}
...
export const resendOTP = (setSnackbarText: any): AsyncAction => async (dispatch) => {
    try {
        dispatch(start())
...
export const verifyOTP = (otp: string, navigate: ReturnType<typeof useNavigate>, setSnackbarText: any): AsyncAction => async (dispatch) => {
    try {
        dispatch(start())
        const email = JSON.parse(localStorage.getItem('email') || '{}')
        const userData = { email: email?.email, otp }
